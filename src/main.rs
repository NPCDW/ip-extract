...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let file: File = file_tool::download_file(url, path).unwrap_or_else(|e| {
        panic!("download file error {}", e)
    });
    println!("download file successed!");

    file_tool::unzip(&file, path.parent().unwrap()).unwrap_or_else(|e| {
    ...
    let path = Path::new("/data/test/test2/proxifier.txt");
