...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let file: File = file_tool::download_file(url, path)?;

    file_tool::unzip(&file, path.parent().unwrap())?;

    let _list = file_tool::read_csv(&path.parent().unwrap().join(Path::new("IP2LOCATION-LITE-DB1.CSV")))?;
